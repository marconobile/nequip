...
def main(args=None, running_as_script: bool = True):
    ...
    found_restart_file = isdir(f"{config.root}/{config.run_name}") # if yaml key: run_name is already present then restart train
...
def fresh_start(config):
    ...
    _set_global_options(config) # load config in a gloabal var _latest_global_config, set up torch/e3nn/jit params and instanciate builder (?)
    ...
    # = Load the dataset =
    dataset = dataset_from_config(config, prefix="dataset") # type: <class 'nequip.data.dataset.NpzDataset'>
